...
from MsgPanelsCommandDirect import MsgPanelsCommandDirect
import time
...
class LEDPanels():
    def __init__(self):
        ...
        self.commands = {
            ...
            'ident_compress_on': {'id': 0x12, 'args': [],
                                  'help': 'ident_compress_on(), enable compression for identical panel patches'},
            'ident_compress_off': {'id': 0x13, 'args': [],
                                   'help': 'ident_compress_off(), disable compression for identical panel patches'},
            'sync_sd_info': {'id': 0x14, 'args': [], 'help': 'sync_sd_info()'},
            'get_version': {'id': 0x15, 'args': [], 'help': 'get_version()'},
            # Run as a service, not on a message topic.
            'show_bus_number': {'id': 0x16, 'args': [], 'help': 'show_bus_number()'},
            'quiet_mode_on': {'id': 0x17, 'args': [],
                              'help': 'quiet_mode_on(), In this mode, there is no feedback information sent from controller'},
            'quiet_mode_off': {'id': 0x18, 'args': [],
                               'help': 'quiet_mode_off(), In this mode, feedback information from controller will be shown on the GUI'},
            'update_gui_info': {'id': 0x19, 'args': [],
                                'help': 'update_gui_info(), Update gain, offset, and position information.'},
            ...
            'set_ao': {'id': 0x10,
                       'args': [{'nbytes': 1, 'min': 1, 'max': 4, 'unsigned': True},
                                {'nbytes': 2, 'min': -32767, 'max': 32767, 'unsigned': True}],
                       # BUG: should this be signed?
        ...
        if __name__ == '__main__':
            self.services = {}
            self.services['readline'] = self.srvGetVersion = rospy.Service('readline', SrvReadline,
                                                                           self.Readline_callback)
            self.services['get_version'] = self.srvGetVersion = rospy.Service('get_version', SrvGetVersion,
                                                                              self.GetVersion_callback)
        ...
        if (self.serialport is not None):
            try:
                self.serial = serial.Serial(self.serialport, baudrate=921600, rtscts=False, dsrdtr=False,
                                            timeout=5)  # 8N1
                # self.serial = serial.Serial(self.serialport, baudrate=921600, stopbits=1, rtscts=False, dsrdtr=False, timeout=5) # 8N1
    ...
    def DiscoverSerialPort(self):
        ...
        if (serialport == 'unspecified'):
            ...
            p2 = subprocess.Popen(["grep", "FTDI USB Serial Device converter now attached"], stdin=p1.stdout,
                ...
                if (nBytesArg > 1):
                    ...
                    if (args_dict_list[iArg]['unsigned'] == False):
                        ...
                        if (command == 'set_ao'):
                            ...
                            # rospy.logwarn('test')
                            serialbytes_list.extend(self.Signed_2byte_to_chr(arg, nBytesArg))
    ...
    # Ported from Reiser's panel code.
    def Signed_2byte_to_chr(self, byte, nBytesArg):

        if (byte > 32767) or (byte < -32767):
            rospy.logerr('this number is out of range - need a function to handle multi-byte values')

        # this does both pos and neg in one line
        temp_val = (65536 + byte) % 65536
        # char_val = [0] * nBytesArg
        char_val = self.Dec2chr(temp_val, nBytesArg)
        # for cnt in range(len(temp_val)):
        #     char_val[2 * cnt - 1:2 * cnt] = self.Dec2chr(temp_val[cnt], nBytesArg)
    ...
    def PanelsCommand_callback(self, panelcommand):
        if self.initialized:
            ...
            serialbytes_list = self.SerialBytelistFromCommand(command,
                                                              [panelcommand.arg1, panelcommand.arg2, panelcommand.arg3,
                                                               panelcommand.arg4, panelcommand.arg5, panelcommand.arg6])
            self.MakeSurePortIsOpen()
    ...
    def PanelsCommand_direct(self, panelcommand):
        if self.initialized:
            # rospy.logwarn('ledpanels: %s(%d,%d,%d,%d,%d,%d)' % (panelcommand.command, panelcommand.arg1, panelcommand.arg2, panelcommand.arg3, panelcommand.arg4, panelcommand.arg5, panelcommand.arg6))
            command = panelcommand.command.lower()
            serialbytes_list = self.SerialBytelistFromCommand(command,
    ...
    def reset(self): # reset controller
        panelcommand = MsgPanelsCommandDirect('ctr_reset', (0, 0, 0, 0, 0, 0))
        self.PanelsCommand_direct(panelcommand)
        time.sleep(self.panel_delay)

    def stop(self): # stop pattern
        panelcommand = MsgPanelsCommandDirect('stop', (0, 0, 0, 0, 0, 0))
        self.PanelsCommand_direct(panelcommand)
        time.sleep(self.panel_delay)

    def start(self): # start pattern
        panelcommand = MsgPanelsCommandDirect('start', (0, 0, 0, 0, 0, 0))
        self.PanelsCommand_direct(panelcommand)
        time.sleep(self.panel_delay)

    def set_pattern_id(self, patid=1): # set pattern ID
        panelcommand = MsgPanelsCommandDirect('set_pattern_id', (patid, 0, 0, 0, 0, 0))
        self.PanelsCommand_direct(panelcommand)
        time.sleep(self.panel_delay)

    def set_position(self, xpos=1, ypos=1): # set pattern position
        panelcommand = MsgPanelsCommandDirect('set_position', (xpos, ypos, 0, 0, 0, 0))
        self.PanelsCommand_direct(panelcommand)
        time.sleep(self.panel_delay)

    def set_mode(self, xmode=0, ymode=0): # set mode
        panelcommand = MsgPanelsCommandDirect('set_mode', (xmode, ymode, 0, 0, 0, 0))
        self.PanelsCommand_direct(panelcommand)
        time.sleep(self.panel_delay)

    def send_gain_bias_16(self, xgain=0, xbias=0, ygain=0, ybias=0): # set gain & bias
        panelcommand = MsgPanelsCommandDirect('send_gain_bias_16', (xgain, xbias, ygain, ybias, 0, 0))
        self.PanelsCommand_direct(panelcommand)
        time.sleep(self.panel_delay)

    def set_posfunc_id(self, channel=0, funcid=0):  # set position function ID
        # channel = 0 (x) , channel = 1 (y)
        panelcommand = MsgPanelsCommandDirect('set_posfunc_id', (channel, funcid, 0, 0, 0, 0))
        self.PanelsCommand_direct(panelcommand)
        time.sleep(self.panel_delay)

    def set_velfunc_id(self, channel=0, funcid=0):  # set velocity function ID
        # channel = 0 (x) , channel = 1 (y)
        panelcommand = MsgPanelsCommandDirect('set_velfunc_id', (channel, funcid, 0, 0, 0, 0))
        self.PanelsCommand_direct(panelcommand)
        time.sleep(self.panel_delay)

    def set_funcx_freq(self, freq):  # set x-channel update frequency (hz)
        # channel = 0 (x) , channel = 1 (y)
        panelcommand = MsgPanelsCommandDirect('set_funcx_freq', (freq, 0, 0, 0, 0, 0))
        self.PanelsCommand_direct(panelcommand)
        time.sleep(self.panel_delay)

    def set_funcy_freq(self, freq):  # set y-channel update frequency (hz)
        # channel = 0 (x) , channel = 1 (y)
        panelcommand = MsgPanelsCommandDirect('set_funcy_freq', (freq, 0, 0, 0, 0, 0))
        self.PanelsCommand_direct(panelcommand)
        time.sleep(self.panel_delay)

    def setArena(self, patID, patPos, mode, gain_bias):
        self.stop()
        self.set_pattern_id(patID)
        self.set_position(*patPos)
        self.set_mode(*mode)
        self.send_gain_bias_16(*gain_bias)
        self.start()
...
if __name__ == '__main__':
    ...
    if panels.initialized:
        ...
        # for k in range(5):
        #     gain_bias = (20, 0, 0, 0)
        #     panels.setArena(patID, patPos, mode, gain_bias)
        #     time.sleep(5)
        #
        #     gain_bias = (-20, 0, 0, 0)
        #     panels.setArena(patID, patPos, mode, gain_bias)
