...
class LEDPanels():
    def __init__(self):
        ...
        print('ledpanels using %s' % self.serialport)
    ...
    def Signed_2byte_to_chr(self, byte, nBytesArg):
        ...
        if (byte > 32767) or (byte < -32767):
            print('this number is out of range - need a function to handle multi-byte values')
    ...
    def PanelsCommand_callback(self, panelcommand):
        if self.initialized:
            ...
            try:
                ...
                print('ledpanels: Serial exception, trying to reopen: %s' % e)
                if self.serial.isOpen():
                    self.serial.close()
                self.MakeSurePortIsOpen()
                try:
                    ...
                    print('ledpanels: Write FAILED to serial port %s: %s' % (self.serialport, e))
                else:
                    print('ledpanels: Write succeeded to serial port %s.' % self.serialport)
